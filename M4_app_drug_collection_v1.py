    ...
    [
        sg.Column(
                [
                    ...
                    [sg.Listbox(values=[],size =(42, 17),key ='-DRUGS-',enable_events=True)],
...
def refresh_collection_page():
  if empl_collection.logged_on_employee:#only authorised employee can see collection
    record_list,start_idx,end_idx=drug_collection.get_page(sort=[("TRADENAME",1)])
    window['-DRUGS-'].Update(values=record_list)
    window['-PG-'].Update(value=f"{start_idx+1}-{end_idx+1} of {drug_collection.doc_count}")

def set_drug_focus(drug):
    #refreshes the collection and selects the drug in the listbox
    page_num,page_pos=drug_collection.get_doc_pos(drug.get('_id'),{},sort=[("TRADENAME",1)])
    print(f'Updated drug is on page:{page_num}, at index:{page_pos}')
    #clear filter
    window['-FILTER-'].update(value='')
    drug_collection.reset_filter()
    drug_collection.set_page(page_num)            
...
def clear_ui():#when logging out
    ...
    window['-DRUGS-'].update(set_to_index=[-1])
    window['-DRUGS-'].update(values=[])
    window['-FILTER-'].update(value='')
    drug_collection.reset_filter()
    window['-LABEL_GRAPH-'].erase()
    window['_id'].update(value='')
    window['-PHOTO_GRAPH-'].erase()
    for f in DrugGUI.field_list:
...
while True:    
    if not empl_collection.logged_on_employee:
        ...
        if event=='Exit' or event ==sg.WIN_CLOSED:
            ...
            drug_collection.set_filter({'TRADENAME':{'$regex':f'^{name_pattern}','$options':'i'}})
            drug_collection.first_page()
            refresh_collection_page()

        elif event in ['⇤','←','→','⇥','↺']:
            if event=='⇤':
                do_update=drug_collection.first_page()
            if event=='←':
                do_update=drug_collection.prev_page()
            elif event=='→':
                do_update=drug_collection.next_page()
            elif event=='⇥':
                do_update=drug_collection.last_page()
            elif event=='↺':
            ...
            drug=drug_collection.get_selected()
            if drug:
                print(f"updating record of {drug}")   
            ...
            if drug:
                ...
                if filename!='':
                    oid=MiniMongo.uploadGridFSFile(filename,tag="drug_photo")
                    if oid:                
            ...
            drug=drug_collection.get_selected()
            if drug:            
                DrugGUI.print_label(window['-LABEL_GRAPH-'],drug)            
