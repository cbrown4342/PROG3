...
# Fix this!!!!
class DrugGUI(Drug):
    #field list inherited from the Drug class (MiniMongoDocument)
    field_defs={
        'ATC':{
            'label':'ATC',
            'desc':'ATC',
        ...
        'DIN':{
            'label':'DIN',
            'desc':'drug identification number',
            'type':str,
            'ui':psg.InputText},
        'PRO':{
            'label':'Province',
    ...
    def update(cls,window,drug):
        ...
        #values=drug.doc
        window['_id'].update(value=drug.get('_id'))
        for k in cls.field_defs:
            print(f'current doc value for key {k} is:{drug.get(k)}')
            if k in drug.doc:
                if k in cls.field_defs:
                    data_type=cls.field_defs[k]['type']                
                    current_value=drug.get(k)
    ...
    def draw_label(cls,graph,drug):
        ...
        graph.DrawText(drug.get("TRADENAME"),(10,16),font=SML_FNT,text_location = psg.TEXT_LOCATION_TOP_LEFT)
        # graph.DrawText(drug.get('gender'),(10,26),font=SML_FNT,text_location =  psg.TEXT_LOCATION_TOP_LEFT)
        graph.DrawText(drug.get('purchase_date'),(10,36),font=SML_FNT,text_location =  psg.TEXT_LOCATION_TOP_LEFT)
        # graph.DrawText(drug.get('allergies'),(150,16),font=SML_FNT,text_location =  psg.TEXT_LOCATION_TOP_LEFT)
        #barcode      
        barcode_data="DIN"+str(drug.get('DIN'))
    ...
    @classmethod
    def print_label(cls,graph,drug):
        print(f"printing wristband of {drug}")   
        #prints graphic to a pdf; file name will start with the string DIN and will include the actual DIN value
        pdf_file=cg.graph_to_pdf(graph,f"DIN{drug.get('DIN')}")
