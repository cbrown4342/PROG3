...
class Drug(MiniMongoDocument):
    ...
    def new(cls):
        new_doc=super().new()
        print('New drug doc:',str(new_doc))
        new_drug=Drug(new_doc)
        return new_drug
    ...
    def __str__(self):
        return f"{self.get('TRADENAME')} ({self.get('ATC')}) ({self.get('PRO')}) ({self.get('SA')})" 
